...
<task type="auto">
  ...
  <files>global.json, .gitignore, package.json</files>
  <action>
...
CRITICAL: Use React 18.3.1 (NOT React 19) per RESEARCH.md - Feliz 2.x has key warnings with React 19.
  ...
  <verify>
Files exist:
- ls global.json .gitignore package.json
- cat global.json | grep "10.0.100"
- cat package.json | grep "18.3.1"
  </verify>
  <done>
global.json exists with .NET 10.0.100, .gitignore covers .NET + Node + Vite, package.json has React 18.3.1 and Vite 7
  </done>
</task>
...
<success_criteria>
1. All 8 files created (global.json, .gitignore, package.json, App.fsproj, App.fs, Main.fs, vite.config.js, index.html)
2. Version pinning follows RESEARCH.md: .NET 10, React 18.3.1, Fable 4.3.0
3. Elmish.HMR opened last in Main.fs (HMR will work)
4. Vite plugin order correct: fable before react (transpilation will work)
5. HTML uses import statement (Vite can recognize .fs extension)
6. build.sourcemap: true in vite.config.js (source maps will work in production)
</success_criteria>
