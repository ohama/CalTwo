...
must_haves:
  truths:
    ...
    - "Production build generates static files (dist/ with HTML + JS)"
  ...
  artifacts:
    ...
    - path: "dist/"
      provides: "Production build output (verified, then cleaned)"
      contains: "index.html"
...
<task type="auto">
  <name>Verify production build generates static files</name>
  <files>dist/</files>
  <action>
...
ls dist/assets/*.js && echo "JavaScript bundles exist"
ls dist/assets/*.css 2>/dev/null || echo "CSS files optional (inline styles OK)"
```

Verify index.html references bundled assets:
```bash
grep -E 'src=".*\.js"' dist/index.html && echo "JS bundle referenced"
```

Confirm build completed without errors by checking exit code of npm run build (already verified above).

Clean up dist/ after verification (optional, keeps workspace clean):
```bash
rm -rf dist
```

EXPECTED BEHAVIOR:
1. npm run build completes without Fable compilation errors
2. dist/ directory created with index.html
3. dist/assets/ contains .js files (bundled app code)
4. index.html references assets with correct paths
5. No "Cannot find module" or TypeScript errors

If build fails:
- Check for F# syntax errors in App.fs or Main.fs
- Verify vite.config.js has correct build settings
- Check package.json has "build": "vite build" script
- Ensure all dependencies installed (npm install completed)

This verifies DEP-01 requirement: "App builds to static files (HTML + JS + CSS)".
  </action>
  <verify>
Verification steps:
1. npm run build completes with exit code 0
2. test -d dist returns true
3. test -f dist/index.html returns true
4. ls dist/assets/*.js finds at least one .js file
5. grep 'src=".*\.js"' dist/index.html finds bundle reference
  </verify>
  <done>
Production build completes successfully, dist/ directory exists with index.html and bundled JavaScript assets, build process completes without errors
  </done>
</task>
...
6. Production build generates dist/ with index.html and bundled JS
7. Korean tutorial created with 100+ lines, all code examples, troubleshooting guide
</verification>
...
6. npm run build generates dist/index.html and dist/assets/*.js
7. Build completes without Fable compilation errors
8. tutorial/phase-01.md exists with comprehensive Korean content
</success_criteria>
